class PostsController < ApplicationController
  ...
  def month_index
   @monthly_posts = Post.page(params[:page]).per(5).select("pay_month").where(user_id: params[:id]).group("pay_month").order(pay_month: "desc")
  end
  ...
  def monthly_total
   ...
   @warikan = @total / 2
   @user_total = @posts.where(pay_name: "1").sum("payment")
   @user_pay = @user_total - @warikan
   @partner_total = @posts.where(pay_name: "2").sum("payment")
