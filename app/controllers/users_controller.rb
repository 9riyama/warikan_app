class UsersController < ApplicationController
  ...
  def show
    ...
    @users = @user.posts.page(params[:page]).per(5).where(pay_name: "1").order('updated_at DESC')
  end
  
  def partner_pay
    @user = User.find(params[:id])
    @users = @user.posts.page(params[:page]).per(5).where(pay_name: "2").order('updated_at DESC')
  end
